...
class Archive(Generic[T]):
    ...
    def load_from_dict(self, data: Dict[str, List[T]]) -> None:
        ...
        """
        archive = {ast.literal_eval(k): v for k, v in data.items()}

        # When the first element of the value is list then concatenate the list
        # [[a,], b, c, d] should be converted [a, b, c, d]
        self._archive = {k: v if not isinstance(v[0], list) else v[0] + v[1:] for k, v in archive.items()}
